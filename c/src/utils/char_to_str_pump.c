...
bool char_to_str_pump(char *str, char c, int *str_size, bool debug)
{
    ...
    if (*str_size==0)
    {
        ...
        if (str==NULL)
        {
            printf("ERROR (%s):%d :: Memory allocation failed for state name\'s first character.\n", file, __LINE__);
            return false;
        }
        else
        {
            *str_size++;            // Incrementing the size of string
            ...
            if (debug==true)
            {
                printf("OK (%s):%d :: Memory allocated successfully for state name\'s first character.\n", file, __LINE__);
            }
        }
    }
    else if (*str_size>0)
    {
        ...
        if (alloc_ret==NULL)
        {
            printf("ERROR (%s):%d :: Memory allocation failed for state name\'s remaining character.\n", file, __LINE__);
            return false;
        }
        else
        {
            *str_size++;            // Incrementing the size of string
            ...
            if (debug==true)
            {
                printf("OK (%s):%d :: Memory allocated successfully for state name\'s remaining character.\n", file, __LINE__);
            }
        }
    }
